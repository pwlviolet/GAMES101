...
const int beziercount = 8;
void mouse_handler(int event, int x, int y, int flags, void *userdata) 
{
    if (event == cv::EVENT_LBUTTONDOWN && control_points.size() < beziercount)
    {
...
void bezier2(const std::vector<cv::Point2f>& control_points, cv::Mat& window)
{
    ...
    for (float t = 0.0; t <= 1; t += 0.001)
    {
        auto point = recursive_bezier(control_points, t);
        window.at<cv::Vec3b>(point.y, point.x)[1] = 255;
        int x = point.x, y = point.y;// ÏñËØÎ»ÖÃ
...
int main() 
{
    ...
    while (key != 27) 
    {
        ...
        if (control_points.size() == beziercount)
        {
