                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    //当ray的各项为负数时，说明光是反方向的，则min和max要互换
    ...
    //求出三个对面的最小和最大的进入时间，作为光线在物体内传播的时间。
    float t_min = std::max(t_xmin, std::max(t_ymin, t_zmin));
    float t_max = std::min(t_xmax, std::min(t_ymax, t_zmax));
    //当且仅当出去>进入，出去>0时相交。
